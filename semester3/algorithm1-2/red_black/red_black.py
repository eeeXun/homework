...
class RBBinaryTree(object):
    ...
    def rb_insert(self, value):
        ...
        while True:
            ...
            if parent_node.color == 'red':
                ...
                # 如果父节点为红节点且叔节点为黑节点
                if uncle_node == None or uncle_node and uncle_node.color == 'black':
                    if parent_node == grandparent_node.left_child:
    ...
    def show_tree(self):
        if self.is_empty():
            ...
            return
        print('-' * 20)
        print(" {}".format(str(self.root.data))) if self.root.color == 'red' else print(str(self.root.data))
        canvas.delete("all")
        canvas.create_oval(380,60,420,100)
        canvas.create_text(400,77,fill="darkblue",font="Times 20 italic bold",text=str(self.root.data))
        self.__print_tree(self.__root, 400, 100, 1)
        print('-' * 20)

    def __print_tree(self, node, x, y, l, prefix=None):
        if prefix is None:
        ...
        if self.has_child(node):
            if node.right_child is not None:#>>>>>>>>>>>>>>>
                if node.right_child.color == 'red':
                    print(prefix + self.prefix_branch + self.prefix_right + " {}".format(str(node.right_child.data)))
                    self.draw(x,y,'r',str(node.right_child.data),l)
                else:
                    print(prefix + self.prefix_branch + self.prefix_right + str(node.right_child.data))
                    self.draw(x,y,'r',str(node.right_child.data),l)
                if self.has_child(node.right_child):
                    self.__print_tree(node.right_child,x+(200/l),y+60, l+1, prefix + self.prefix_branch + ' ')
            else:
                print(prefix + self.prefix_branch + self.prefix_right)
            if node.left_child is not None:#<<<<<<<<<<<<<<<<
                if node.left_child.color == 'red':
                    print(prefix + self.prefix_leaf + self.prefix_left + " {}".format(str(node.left_child.data)))
                    self.draw(x,y,'l',str(node.left_child.data),l)
                else:
                    print(prefix + self.prefix_leaf + self.prefix_left + str(node.left_child.data))
                    self.draw(x,y,'l',str(node.left_child.data),l)
                if self.has_child(node.left_child):
                    prefix_left_child += '  '
                    self.__print_tree(node.left_child,x-(200/l),y+60, l+1, self.prefix_leaf + prefix_left_child)
            else:
...
if __name__ == '__main__':
    ...
    print(tree.root.left_child.color)
